...
grid_1 = [45,30] * np.array(grid_generator(5,5)) + [-12,5]
...
kernel_centers = mean_shift_cluster(grid_1, Y, K, 10**(-1), 2.0)
...
kernel_centers = remove_redundant(kernel_centers, 10**(-1))
print("# of kernels: " + str(kernel_centers.shape[0]))
...
for k in np.arange(grid.shape[0]):
	ker3[k] = kernels_definer(k, grid, 3.0)

for i in np.arange(grid.shape[0]):
	for j in np.arange(2):
		J_ker3[i,j] = kernels_grad_definer(i,j, grid, 3.0)

(M, J_M) = first_and_second_moments()

G = np.concatenate((ker1, ker2,ker3))
J_G = np.concatenate((J_ker1, J_ker2, J_ker3), axis=0)
...
(a,b,c) = local_OT_solver(G,J_G,X,Y, 0.04, s0, 1)


plt.figure()
plt.subplot(211)
plt.title('Slightly Less Simple Optimal Transport')
