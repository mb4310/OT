...
I_2 = (misc.imresize(misc.imread(image_2), I_1.shape))
...
def optimal_map(y):
	return y + np.sum(np.dot(z,evaluate_functions(J_G,y)), axis=0)
...
I_transformed = reconstruct_image(X_transform, I_1.shape[0], I_1.shape[1], np.amax(np.array(I_1)))
